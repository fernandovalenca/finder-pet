...
const logos = {
  google: () => <LogoGoogle className="h-5 w-5 fill-neutral-600" />,
  twitter: () => <LogoTwitter className="h-5 w-5 fill-neutral-600" />,
  facebook: () => <LogoFacebook className="h-5 w-5 fill-neutral-600" />,
};
...
export const SocialButton = forwardRef<HTMLButtonElement, SocialButtonProps>(
  ({ children, logo, className, ...rest }, ref) => {
    ...
    return (
      <button
        ref={ref}
        className={`w-full flex items-center justify-between py-2 px-3 border border-neutral-500 rounded-lg ${className}`}
        {...rest}
      >
